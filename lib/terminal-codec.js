...
import { toolResultText } from './codec.js'
...
/** Full text of a dsh tool result, or `undefined` when the tool failed. The
 *  content flattening is shape-tolerant across dsh lines (see
 *  `codec.toolResultText`). */
export function resultText(event) {
  if (event.data.error !== undefined) return undefined
  return toolResultText(event.data.message?.content)
}
