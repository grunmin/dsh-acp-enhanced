...
/**
 * Fold the latest committed context capacity out of a session's log.
 *
 * The harness appends `request/context` only when the provider, model,
 * capacity, or `systemPromptUpdate` mode *changes* (see dsh-session's
 * `requestContext()` fold), so a session resumed from disk already carries the
 * event and the next request never re-emits it. A listener that only watches
 * the live event therefore leaves a resumed session with no capacity, and every
 * usage sample reports `size === used` — an editor ring pinned at 100% no
 * matter how little is actually in context. Folding the committed log closes
 * that gap with the same generation-agnostic plain-events shape as
 * `runningPresetFrom`, so the live and stored reads cannot disagree.
 *
 * The **latest** `request/context` is authoritative: a route that stops
 * declaring capacity must not fall back to an older route's window. The
 * positive-integer guard is deliberately stricter than the harness's verbatim
 * fold, which would carry a malformed `0` straight into a zero-sized ring;
 * here such a value reads as *no* denominator (`size === used`) instead.
 * @param events - the session's committed events, in log order.
 * @returns the latest positive integer context window, or `undefined`.
 */
export function contextWindowFrom(events) {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index]
    if (event?.type !== 'request/context') continue
    const contextWindow = event.data?.contextWindow
    return Number.isInteger(contextWindow) && contextWindow > 0 ? contextWindow : undefined
  }
  return undefined
}
