 ...
 * text-kind: ACP carries no attachment bytes, so they become the same textual
 * placeholder `textFromBlocks` uses.
 *
 * @param content - a dsh `assistant/message` content block array.
 * @returns ordered `{ kind: 'text' | 'thought', text }` chunks; blank ones dropped.
 */
