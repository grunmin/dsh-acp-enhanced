...
import { assistantContentChunks, attachmentIngestOf, acpPromptCommandImages, acpPromptLineText, convertPrompt, PromptImageError, sanitizeWireTitle, turnEndToStopReason, UnsupportedPromptContentError, usageTelemetry } from './codec.js'
...
export function apply(ctx, config) {
  ...
  function emitMessageFallback(record, event) {
    ...
    // Order matters: the committed message carries reasoning *before* the text
    // it produced, and the live seam streams it that way. Emitting text first
    // would render the thinking block below the reply it belongs to — on the
    // session's final message, a think block parked after the report. The
    // per-kind guards below preserve the partial-streaming behaviour either way.
  ...
  /** Extract reasoning text from assistant content blocks (fallback delivery). */
