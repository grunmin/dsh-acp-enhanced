...
import { assistantContentChunks, attachmentIngestOf, acpPromptCommandImages, acpPromptLineText, convertPrompt, fallbackDelivery, PromptImageError, sanitizeWireTitle, turnEndToStopReason, UnsupportedPromptContentError, usageTelemetry } from './codec.js'
import { isTerminalToolName, parseShellExitStatus, resultText, shellCallCwd, stripShellPrefix, terminalCommandLine, toolKindFor } from './terminal-codec.js'
      ...
      try {
        switch (event.type) {
          ...
          case 'assistant/message': {
            ...
            // block at the very end of the thread.
            for (const chunk of assistantContentChunks(event.data.message.content)) {
              await notifyNow(record, {
                sessionUpdate: chunk.kind === 'text' ? 'agent_message_chunk' : 'agent_thought_chunk',
                messageId: randomUUID(),
                content: { type: 'text', text: chunk.text },
              })
            }
