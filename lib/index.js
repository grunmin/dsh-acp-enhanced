 ...
 *
 * Zed treats kind == 'execute' as a TERMINAL tool and kind == 'edit' as a
 * diff tool, and for both it HIDES rawInput (the "Raw Input:" section). Only
 * genuinely terminal tools (zed_terminal) should be 'execute'; local executors
 * like bash/run_code must stay 'other' so the editor shows the exact arguments
 * (the command) plus the output. Write/edit tools likewise stay 'other' unless
 * the bridge also emits diff content — otherwise the card would show nothing.
 */
...
function toolCallTitle(name, argumentsValue) {
  ...
  if (name === 'zed_terminal' || /bash|pwsh|powershell|shell|exec|run_code|execute|terminal/.test(name)) {
...
export function apply(ctx, config) {
  ...
  async function applySelection(record, next) {
    let resolved
    try {
      resolved = await llm.resolveCallConfig(next)
    } catch (error) {
      // A reasoning effort can outlive the model that supported it: Zed
      // re-applies its saved `default_config_options` on every session, and
      // switching models carries the session's current effort onto the newly
      // picked model, which may not offer it (glm-* and gpt-5.6-luna reject
      // "max"). Rejecting the whole switch snapped Zed's dropdown back and
      // made model switching itself appear broken, so drop the stale effort
      // and resolve against the target model's own default instead. Any other
      // failure (unknown route, missing credential) still surfaces.
      if (error?.code !== 'UNSUPPORTED_REASONING_EFFORT' || next.reasoningEffort === undefined) throw error
      logger.warn(`acp-enhanced: dropped reasoning effort "${next.reasoningEffort}" for ${next.provider}/${next.model} (unsupported); using the model default`)
      resolved = await llm.resolveCallConfig({ ...next, reasoningEffort: undefined })
    }
    const selected = {
