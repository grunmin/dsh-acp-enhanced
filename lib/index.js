...
export function apply(ctx, config) {
  ...
  /** Stored session headers, unwrapping the `{header, …}` list snapshot. */
  async function storedSessionHeaders(persistence) {
  ...
  async function readStoredTitle(persistence, id) {
    try {
      // `stat()` is the size guard: an oversized artifact is skipped rather
      // than fully decoded.
      const snapshot = await persistence.stat(id)
      if (snapshot === undefined) return undefined
      if (snapshot.sizeBytes !== undefined && snapshot.sizeBytes > 8 * 1024 * 1024) return undefined
      const { events } = await loadStoredSession(persistence, id)
  ...
  const makeAgent = (connection) => {
    ...
    return {
      ...
      async listSessions(params) {
        ...
        const persistence = ctx.get('sessionPersistence')
        const headers = persistence === undefined ? [] : await storedSessionHeaders(persistence)
        const out = []
