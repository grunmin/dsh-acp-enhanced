...
export function apply(ctx, config) {
   ...
   *  rather than resolved through the agent-preset registry so the read is
   ...
   *  map it to invalid params. The registry reports both through `RemoteError`
   ...
   *  legal while it is blank (agent-preset registry product rule): swapping tools
  ...
  async function composePreset(requested) {
    ...
    const resolvedId = (await presets.resolve(requested)).id
    return {
  ...
  /** The /preset command: list the registry or switch by exact id/substring. */
  async function presetCommandText(record, query) {
    const presets = agentPresets()
    if (presets === undefined) return 'no agent-preset registry is mounted in this profile'
