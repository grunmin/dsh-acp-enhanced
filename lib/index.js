 ...
 *   carries torn text. The live seam is the agent-scoped
 *   `agent/assistant-stream` frames event; a host that emits no frames — or
 *   whose frames were dropped — is covered by a fallback: when a step streamed
 *   nothing, the committed `assistant/message` is forwarded, so the reply is
 *   never lost — and hosts that do stream are untouched, because the fallback
 *   only fires on silence.
...
import { budgetFrom, StoredTitleReader } from './stored-titles.js'
...
// booting CLI pins a different generation. The preset roster is reached
// through `ctx.agentPresets` only — never through a package import — and its
// failure modes are identified structurally (see isPresetClientError).
import { renderSkillContent } from '@deepseek-ai/dsh-skill'
import { defineTool } from '@deepseek-ai/dsh-tools'
import { SessionId } from '@deepseek-ai/dsh-session'
import { assistantContentChunks, attachmentIngestOf, acpPromptCommandImages, acpPromptLineText, contextWindowFrom, convertPrompt, fallbackDelivery, isBlankFrom, PromptImageError, runningPresetFrom, sanitizeWireTitle, toolResultCallId, toolResultText, turnEndToStopReason, UnsupportedPromptContentError, usageTelemetry } from './codec.js'
import { isTerminalToolName, nameHasKeyword, parseShellExitStatus, resultText, shellCallCwd, stripShellPrefix, terminalCommandLine, toolKindFor } from './terminal-codec.js'
...
export const Config = Schema.object({
   ...
   *  `agent/assistant-stream` chunk) from a hung tool call (long `tool/call` →
 ...
 *  Whole-segment matching (see `nameHasKeyword`) — a bare substring is not a
 *  class signal (`dispatch` is not an executor just because it contains
 *  `exec`); multi-segment keywords (`run_code`) match consecutive segments. */
const EXECUTOR_KEYWORDS = ['bash', 'pwsh', 'powershell', 'shell', 'exec', 'execute', 'executor', 'run_code', 'terminal']
const isExecutorName = (name) => nameHasKeyword(name, EXECUTOR_KEYWORDS)

/** Fenced-code language hint per executor tool (best effort — highlight only). */
function executorLang(name) {
  if (nameHasKeyword(name, ['pwsh', 'powershell'])) return 'powershell'
  if (nameHasKeyword(name, ['run_code'])) return 'typescript'
  return 'bash'
}
...
function toolCallContentFor(name, argumentsValue, kind) {
  ...
  if (name !== 'zed_terminal' && isExecutorName(name)) {
...
function toolCallTitle(name, argumentsValue) {
  ...
  // Editor-run commands (`zed_terminal`) take a program plus argv —
  // `{command: 'bash', args: ['-lc', '<line>'], cwd}` — so `command` alone is
  // the evaluator's name and a card titled from it reads "bash". Show the
  // command line; kind 'execute' renders this label as plain text.
  if (name === 'zed_terminal') {
    const line = terminalCommandLine(obj)
    if (line !== undefined) return clipOneLine(line, 120)
  }

  // Execute-kind tools (plain-text label): show the command itself.
  if (isExecutorName(name)) {
    // Local executors like bash/run_code are kind 'other' and render as
    ...
    if (command) return clipOneLine(mdEscape(command), 100)
    const argv = obj.args
    if (Array.isArray(argv) && argv.length > 0) return clipOneLine(mdEscape(argv.map(String).join(' ')), 100)
    const cwd = str('cwd')
    if (cwd) return clipOneLine(mdEscape(`Run in ${cwd}`), 100)
  }

  // File-content tools: `Read <path>` (incl. line range when present).
  if (nameHasKeyword(name, ['read', 'cat', 'show', 'view'])) {
    const path = pathArg(obj)
    if (path) {
  ...
  if (nameHasKeyword(name, ['write', 'edit', 'editor', 'patch', 'apply', 'replace'])) {
    const path = pathArg(obj)
    if (path) return clipOneLine(`Write ${mdEscape(path)}`, 120)
  }

  // Content search: `Search: <pattern>`.
  if (nameHasKeyword(name, ['grep', 'rg', 'content_search', 'search_text'])) {
  ...
  if (nameHasKeyword(name, ['glob', 'find'])) {
  ...
  if (nameHasKeyword(name, ['fetch', 'http'])) {
  ...
  if (nameHasKeyword(name, ['ask', 'question', 'elicit']) && Array.isArray(obj.questions)) {
  ...
  if (nameHasKeyword(name, ['spawn_agent', 'subagent', 'spawn', 'delegate', 'task'])) {
...
function resultPreview(event) {
  ...
  const text = toolResultText(event.data.message?.content)
...
export function apply(ctx, config) {
  ...
  // `approval` is not captured: `permission.set()` owns the approval knob, and
  // the service is required through `inject` (its `approval/request` waterfall
  // is answered below).
  ...
  /** ACP session modes for every switchable permission preset, in table order.
   *  Built from the service's own `optionOf`, so labels and descriptions can
   *  never drift from the `permission_preset` config option. */
  const permissionModes = (permission) => permission.names.map((presetName) => {
    const option = permission.optionOf(presetName)
    return {
      id: option.value,
      name: option.name,
  ...
  /**
   * Read one session's committed events. `snapshotEvents()` returns a cached
   * frozen snapshot, invalidated on every append; the session exposes
   * `ownEvents()` / `eventAt()` for the other shapes.
   */
  const sessionEventsOf = (session) => session.snapshotEvents()
   ...
   *  `agent-preset/selected` event wins over the creation header (the fold
   *  lives in `codec.runningPresetFrom`, shared with the stored-log read in
   *  `persistedFactsOf` — both take an events array, so the two paths cannot
   *  disagree on shape). */
  const runningPresetOf = (session) => runningPresetFrom(sessionEventsOf(session), session.header)

  /** A session's context capacity, read from its committed log. The live
   *  `request/context` listener keeps `record.contextWindow` current on a route
   *  or capacity change, but a resumed session never re-emits an unchanged
   *  `request/context` — so without this fold its first usage sample would
   *  report `size === used` (the editor ring pinned at 100%). The fold is the
   *  same one the stored-log read uses (`codec.contextWindowFrom`). */
  const contextWindowOf = (session) => contextWindowFrom(sessionEventsOf(session))
   ...
   *  map it to invalid params. The roster reports both through `RemoteError`
   *  with a stable `agent-preset/*` code (`not-found`, `invalid`, `locked`),
   *  identified structurally through the `isDSHRemoteError` marker — never
   *  instanceof, because that class lives in the host tree and a second copy
   *  of it in this package's closure would fail the check. */
  const isPresetClientError = (error) => {
    if (!(error instanceof Error)) return false
    return error.isDSHRemoteError === true
      && typeof error.code === 'string'
      && error.code.startsWith('agent-preset/')
  }
   ...
   *  make. Delegates to `codec.isBlankFrom`, shared with the stored-log read in
   *  `persistedFactsOf`. */
  const isBlankSession = (session) => isBlankFrom(sessionEventsOf(session))

  /**
   * Resolve a preset that is about to compose an agent, refusing an unusable one
   * with the reason discovery recorded.
   *
   * The roster's "resolve the one that can actually mount" call was a private
   * `resolveMountable` through 0.1.6; 0.1.7's `dsh-agent-preset-registry`
   * publishes no such member at all. There, `resolve()` returns a broken preset
   * deliberately — deleting, reading and reporting one all need the row — and
   * every mounting path refuses it *after* resolution. So the refused-here
   * contract is reproduced with the roster's own `broken` verdict, which both
   * lines publish on every row (`AgentPreset.broken`).
   *
   * Probing the member, not the generation: nothing else distinguishes the two
   * rosters at this call site, and a host that republishes `resolveMountable`
   * keeps its own (stricter, discovery-time) refusal.
   *
   * @param presets - the mounted roster.
   * @param requested - preset id, or undefined for the roster default.
   * @returns the resolved, mountable preset.
   * @throws an unknown id (isPresetClientError — mapped by callers) or a broken
   *   preset's discovery-reported reason (a deployment fault).
   */
  async function resolveMountablePreset(presets, requested) {
    if (typeof presets.resolveMountable === 'function') return presets.resolveMountable(requested)
    const preset = await presets.resolve(requested)
    if (preset.broken !== undefined) {
      throw new Error(`agent preset "${preset.id}" cannot compose a session: ${preset.broken}`)
    }
    return preset
  }

  /**
   * The ids worth trying when the preset a deployment named cannot be used, in
   * order: the roster's own default, then the shipped `standard`. Deliberately
   * short — the goal is to keep a stale id from making every session
   * unopenable, not to search for a substitute nobody asked for.
   */
  async function substitutePreset(presets, requested) {
    for (const id of [presets.defaultId, 'standard']) {
      if (typeof id !== 'string' || id === requested) continue
      try {
        return await resolveMountablePreset(presets, id)
      } catch {
        // the next candidate, then give up and let the caller report
      }
    }
    return undefined
  }
  ...
  /**
   * The error a preset that cannot be composed deserves: the roster's own
   * reason, the ids this deployment *does* offer, and the migration that most
   * often explains the miss. Returned as a plain `Error`, so callers stop
   * classifying it as a client mistake — a preset id a deployment configured
   * (or a stored session recorded) is not an invalid parameter, and the old
   * `Unknown agent preset: X` on its own told the user nothing.
   */
  async function presetFailure(presets, requested, error) {
    const detail = error instanceof Error ? error.message : String(error)
    let offered = []
    try {
      offered = (await presets.list()).filter((preset) => preset.broken === undefined).map((preset) => preset.id)
    } catch {
      // the roster's own listing failed; the resolution detail is still evidence
    }
    const roster = offered.length === 0 ? '' : ` This roster offers ${offered.map((id) => JSON.stringify(id)).join(', ')}.`
    return new Error(`${detail}.${roster} A preset named by DSH_ACP_PRESET, by the profile's preset/agent-preset-registry default, or by a stored session can outlive the preset itself: from dsh 0.1.7 the roster scans no user root, so a preset authored under $DSH_HOME/.agent-presets must be declared as an @deepseek-ai/dsh-agent-preset row in the profile's cordis.patch.yml.`)
  }
   ...
   *
   * A named preset can outlive the preset: environment state, not a client
   * request, is what usually names one (`DSH_ACP_PRESET`, a profile default) and
   * 0.1.7 stopped discovering `$DSH_HOME/.agent-presets`, so every user-authored
   * preset disappeared with that line. Refusing the session over a stale string
   * makes the whole panel unusable, so a **blank** session — nothing to strand —
   * composes under a substitute instead, with the miss reported on stderr. A
   * session that already ran keeps its history *and* its preset: composing it
   * under a different tool set is exactly what the blank-only switching rule
   * forbids, so there the failure stands, carrying the roster and the fix.
   *
   * @param requested - preset id, or undefined for the roster default.
   * @param options.blank - whether the session has produced anything yet (a new
   *   session has not; a resumed one says so through its persisted log).
   * @returns `{ agentPreset, setup }`, or `{}` without a roster.
   * @throws the roster's resolution failure, wrapped with context (a deployment
   *   fault — callers map it to an internal error).
   */
  async function composePreset(requested, { blank }) {
    const presets = agentPresets()
    if (presets === undefined) return {}
    let resolved
    try {
      resolved = await resolveMountablePreset(presets, requested)
    } catch (error) {
      if (blank !== true) throw await presetFailure(presets, requested, error)
      const substitute = await substitutePreset(presets, requested)
      if (substitute === undefined) throw await presetFailure(presets, requested, error)
      // stderr only: stdout is the ACP wire. Losing a session to a preset id
      // this deployment no longer has would be worse than saying so.
      process.stderr.write(`dsh-acp-enhanced: agent preset ${JSON.stringify(requested ?? '(roster default)')} is unavailable (${error instanceof Error ? error.message : String(error)}); composing this session under ${JSON.stringify(substitute.id)} instead. /status shows the preset actually in use; from dsh 0.1.7 a user-authored preset must be declared as an @deepseek-ai/dsh-agent-preset row in the profile's cordis.patch.yml.\n`)
      resolved = substitute
    }
  ...
  async function persistedFactsOf(sessionId) {
    ...
    try {
      const { meta, events } = await loadStoredSession(persistence, sessionId)
      return {
        // Plain events-array folds (NOT the live-session adapters): a resumed
        // session's in-memory log is not readable yet, so the stored rows are
        // folded directly here.
        preset: runningPresetFrom(events, meta),
        blank: isBlankFrom(events),
      }
    } catch {
  ...
  function toolResultUpdateFor(record, event, callId, elapsed) {
    ...
    if (!isError && preview !== undefined && call !== undefined
      && call.name !== 'zed_terminal' && isExecutorName(call.name)) {
      content = [{ type: 'content', content: { type: 'text', text: codeFence(preview, executorLang(call.name)) } }]
    } else if (!isError && preview !== undefined && call?.name === 'zed_terminal' && call.terminalId === undefined) {
      // A live editor terminal was embedded in the card and renders its own
      // output; a replayed one (`terminalId` never attached) has no terminal to
      // render, and kind 'execute' hides rawOutput — so surface the captured
      // text as the card body or a reopened thread shows nothing.
  ...
  ctx.on('session/event', (session, event) => {
    ...
    try {
      if (tracePath !== undefined) {
        const extra = event.type === 'tool/call'
          ...
          : event.type === 'tool/result'
            // Neither generation names the tool on the result event: 0.1.5
            // nests the id in a `tool-result` block, 0.1.7 puts it on the
            // message, and `content[0]` carries no `name` in either. The call
            // table still holds it — the switch below deletes the entry.
            ? { name: record.callArgs.get(toolResultCallId(event.data))?.name,
      ...
      if (process.env.ACP_DEBUG) {
        const extra = event.type === 'turn/end' ? ` reason=${JSON.stringify(event.data.reason)}` : event.type === 'agent/inbox/spliced' ? ` hasPending=${record.agent.inbox?.hasPending}` : event.type === 'tool/call' ? ` name=${event.data.name}` : ''
      ...
      switch (event.type) {
        case 'assistant/message':
          ...
          // The committed message is authoritative for this step. Without this
          // fallback a host that streamed nothing would lose the reply
          ...
          // already deliver — in committed order, and including the image
          // blocks that never stream (see `emitMessageFallback`).
        ...
        case 'tool/result': {
          ...
          // The call id moves between generations (0.1.7 message-level, 0.1.5
          // nested on the tool-result block) — see `codec.toolResultCallId`.
        ...
        case 'request/context':
          // The latest route is authoritative: a route that stops declaring
          // capacity (or switches to one that declares none) must clear the
          // cached window rather than inherit the previous route's. The same
          // fold backs the resumed-session fallback in `emitUsage`.
          record.contextWindow = contextWindowFrom([event])
          break
        case 'session/title': {
  ...
  /**
   * Live assistant frames (`agent/assistant-stream`).
   *
   * The harness publishes its `StreamChunk` vocabulary as a process-local,
   * agent-scoped Cordis dispatch: one attempt emits ordered `start` / `chunk` /
   * `end` frames, and `frame.chunk` is the live chunk. Feeding those chunks
   * through `handleChunk` gives block-level streaming and `streamDeltas`. This
   * is the only live seam — `docs/architecture.md` names
   * `agent/assistant-stream` as a live extension point.
   *
   * The dispatch is scope-filtered by the agent in the payload, so a
   * root-context listener receives it for every agent — the official ACP
   * bridge listens to `agent/inbox/claimed` the same way. `ownedRecord`
   * keeps this bridge to its own sessions, so subagent frames are ignored.
   */
  ctx.on('agent/assistant-stream', ({ agent, frame }) => {
    const record = ownedRecord(agent)
    if (record === undefined) return
    // Only the `start` frame carries the turn/step: the `chunk` variant is
    // `{type, attemptId, revision, index, time, chunk}`. Remember them for the
    // attempt so a usage sample that arrives as a chunk can still report which
    // turn/step it belongs to (`emitUsage` reads them off the synthetic event).
    if (frame?.type === 'start') {
      record.streamTurn = frame.turn
      record.streamStep = frame.step
      return
    }
    // `end` only closes the attempt; the chunks are the stream.
    if (frame?.type !== 'chunk') return
    if (process.env.ACP_DEBUG) {
      process.stderr.write(`[acp-debug] agent/assistant-stream frame=chunk chunkType=${frame.chunk.type} turn=${record.streamTurn} step=${record.streamStep}\n`)
    }
    try {
      // The synthetic event carries the turn/step on `data`, which
      // `emitUsage` copies into its `_meta`.
      handleChunk(record, { data: { chunk: frame.chunk, turn: record.streamTurn, step: record.streamStep } })
    } catch (error) {
      logger.warn(`acp-enhanced: assistant frame handling failed: ${String(error)}`)
    }
  })
   ...
   *
   * `handleChunk` is fed by the `agent/assistant-stream` frames event. This
   * fallback is the second tier: a host that emits no frames — or whose frames
   * were dropped — still delivers its reply from the committed
   * `assistant/message`, instead of settling the turn with usage reported and
   * an empty thread. It restores *delivery*, not streaming.
   *
   ...
   *
   * Delivery order comes from `assistantContentChunks`, the same contract
   * history replay uses, so the fallback cannot reorder reasoning against the
   * reply it produced. Images are the one kind that never streams (ACP carries
   * no attachment bytes), so a committed image is always delivered, exactly
   * once, in its committed position.
   */
  function emitMessageFallback(record, event) {
    // `fallbackDelivery` owns the per-kind decision (tested offline); this only
    // forwards what it returns and records it as delivered.
    for (const chunk of fallbackDelivery(event.data?.message?.content, record)) {
      if (chunk.kind === 'thought') record.streamedThought += chunk.text
      else record.streamedText += chunk.text
      notify({
        sessionId: record.agent.session.id,
        update: {
          sessionUpdate: chunk.kind === 'thought' ? 'agent_thought_chunk' : 'agent_message_chunk',
          messageId: record.messageId,
  ...
  /** Cancel pending streamed-delta state for one record at each `step/start`:
   *  block indexes restart per step, so nothing survives a step boundary. */
  ...
  function emitUsage(record, usage, event) {
    ...
    // A resumed session's capacity was logged before this bridge saw it (the
    // harness appends `request/context` only on change), so consult the
    // committed fold once and cache it on the record. The live listener still
    // wins afterward: a route switch re-emits `request/context` and overwrites
    // this before the next sample.
    if (record.contextWindow === undefined) {
      const committed = contextWindowOf(record.agent.session)
      if (committed !== undefined) record.contextWindow = committed
    }
  ...
  async function buildConfigOptions(record) {
    ...
    if (permission !== undefined) {
      options.push({
        ...
        category: 'mode',
        currentValue: permission.current(record.agent.session),
        options: permission.names.map((presetName) => permission.optionOf(presetName)),
      })
    }
    const presets = agentPresets()
  ...
  /** Apply one permission preset, mirroring the /permission command: `set()`
   *  records the selection and writes both knobs (sandbox mode and approval
   *  policy) through their canonical setters. */
  function applyPermissionPreset(record, presetName, permission) {
    ...
    permission.set(record.agent.session, presetName)
  }
  ...
  /**
   * Attach a client-created terminal to its tool card.
   *
   * `zed_terminal` asks the client to create the terminal during execute, so
   * its id does not exist when the tool card is first reported. The card gets
   * it through a `tool_call_update`: embedding a terminal in a tool call is
   * what makes the editor render the terminal and its live output (ACP
   * "Embedding in Tool Calls"), and it keeps displaying after the terminal is
   * released. Without it the execute-kind card has no body — the editor hides
   * `rawOutput` for that kind — so expanding showed nothing.
   */
  function attachClientTerminal(record, callId, terminalId, cwd) {
    if (record === undefined) return
    // Remember the live terminal on the call so the result knows the card has
    // one (and must not duplicate its output as a text body).
    const call = record.callArgs.get(callId)
    if (call !== undefined) call.terminalId = terminalId
    notify({
      sessionId: record.agent.session.id,
      update: {
        sessionUpdate: 'tool_call_update',
  ...
  function syncClientTools() {
    ...
    if (clientCaps.terminal === true) {
      clientToolDisposers.push(tools.register(defineTool({
        ...
        async execute(args, exec) {
          ...
          // The card is reported before the terminal exists; attach it now so
          // the editor renders the live output (see attachClientTerminal).
          attachClientTerminal(ownedRecord(exec.agent), exec.callId, handle.id, args.cwd)
          try {
            let output = ''
            let exitStatus
    ...
    if (clientCaps.elicitation?.form !== undefined && clientCaps.elicitation?.form !== null && userQuestions !== undefined) {
      ...
      // The UI provider that renders those questions as an editor form. It
      // claims each request by answering on the `user-questions/request`
      // Cordis waterfall and delegates anything it does not own to `next()`
      ...
      // Answer the `user-questions/request` waterfall — the declared seam. A
      // listener on the root context is untagged, so the scoped dispatch
      // admits it for every agent; a request this bridge does not own is
      // delegated to the next answerer (falling through to NO_PROVIDER when no
      // one else claims it).
    ...
    if (permission !== undefined) {
      notify({
        ...
        update: {
          ...
          currentModeId: permission.current(record.agent.session),
        },
      })
    }
    broadcastConfig(record).catch((error) => {
  ...
  function makeRecord(handle) {
    ...
    return {
      ...
      streamedText: '',
      streamedThought: '',
      /** The turn/step of the attempt currently being streamed, taken from its
       *  `start` frame (the `chunk` frames do not carry them) so streamed usage
       *  samples can report where they belong. */
  ...
  // ── stored sessions ───────────────────────────────────────────────────────
  //
  // The declared persistence surface is the handle API:
  // `list() → {header, revision, sizeBytes?}[]`, `stat(id)`, and
  // `open(id, 'read') → handle` (`handle.header` + `handle.read()`).

  /**
   * Stored session snapshots, keeping the `{header, revision, sizeBytes}` shape.
   *
   * `list()` is one pass over the whole corpus and already carries the size and
   * the content revision of every session; a per-session `stat(id)` does not
   * add anything the title path needs (and on 0.1.7 it folds an uncached
   * `historicalCorpusRevision` walk into each older-format result). The cost
   * that actually matters is the log **decode**, which is why the title path
   * bounds how many it does — see `lib/stored-titles.js`.
   */
  async function storedSessionSnapshots(persistence) {
    const entries = await persistence.list()
    return entries
      .map((entry) => (entry?.header === undefined ? { header: entry } : entry))
      .filter((entry) => entry.header !== undefined && entry.header !== null)
  }

  /** One stored session's header plus its full event log, through a read handle. */
  async function loadStoredSession(persistence, id) {
  ...
  /** Last stored `session/title` among persisted rows, sanitized for the wire. */
  function lastTitleOf(rows) {
    let last
    for (const row of rows) {
      if (row?.type === 'session/title' && typeof row?.data?.title === 'string') last = row.data.title
    }
    return last === undefined ? undefined : sanitizeWireTitle(last)
  }

  /** The `session/list` deadline for stored titles. Decoding one log is a full
   *  read (`loadStoredSession` → `handle.read()`), so a few hundred sessions
   *  cost minutes; the budget is what keeps the RPC inside its wire timeout.
   *  The deferred pass below finishes the rest off the request path. Parsed
   *  strictly: a `NaN` budget would make the reader's deadline gate fire
   *  never — one typo would restore the unbounded read (see `budgetFrom`). */
  const rawTitleBudget = process.env.DSH_ACP_LIST_BUDGET_MS
  const storedTitleBudgetMs = budgetFrom(rawTitleBudget)
  if (rawTitleBudget !== undefined && storedTitleBudgetMs !== Number(rawTitleBudget)) {
    logger.warn(`acp-enhanced: ignoring DSH_ACP_LIST_BUDGET_MS=${JSON.stringify(rawTitleBudget)}; using ${storedTitleBudgetMs}ms`)
  }

  /** One reader per plugin instance: its cache is what makes the second
   *  `session/list` over an unchanged corpus a map lookup instead of a corpus
   *  read. Built lazily because `loadTitle` closes over the live persistence
   *  service (`ctx.get('sessionPersistence')`), which is only reachable once
   *  the plugin is mounted. */
  let storedTitlesReader

  /**
   * Read one stored log's last title. Only ever called for a snapshot whose
   * `list()` `sizeBytes` passed the reader's guard, and never for a session
   * whose title is already cached.
   */
  async function loadStoredTitle(persistence, snapshot) {
    const { events } = await loadStoredSession(persistence, snapshot.header.id)
    return lastTitleOf(events)
  }

  function storedTitles(persistence) {
    if (storedTitlesReader === undefined) {
      storedTitlesReader = new StoredTitleReader({
        loadTitle: (snapshot) => loadStoredTitle(persistence, snapshot),
        // A title the budget did not reach is still worth publishing: the
        // client already has the list (each entry carries its own timestamp),
        // so it renders the entry bare and fills the label in when this
        // arrives — the same `session_info_update` seam the live
        // `session/title` path uses, minus its `updatedAt` (nothing about the
        // session itself moved).
        onTitle: (snapshot, title) => notify({
   ...
   * steps, usage, plan flips, …) is omitted. Assistant messages replay their
   * reasoning and text runs in generation order (see
   * `assistantContentChunks`), so a replayed thread reads like the live one.
   */
    ...
    for (const event of events) {
      try {
        switch (event.type) {
          ...
          case 'assistant/message': {
            // Emit the committed message in generation order — reasoning before
            // the reply it produced — which is the order the live seam streams.
            // Emitting the text run first would leave the thinking block below
            // its reply, and on the session's final message that parks a think
            // block at the very end of the thread. Image placeholders ride the
            // MESSAGE side (their content belongs to the reply), matching the
            // `assistant/message` fallback's delivery.
            for (const chunk of assistantContentChunks(event.data.message.content)) {
              await notifyNow(record, {
                sessionUpdate: chunk.kind === 'thought' ? 'agent_thought_chunk' : 'agent_message_chunk',
                messageId: randomUUID(),
                content: { type: 'text', text: chunk.text },
    ...
    return {
      initialize(params) {
        ...
        return Promise.resolve({
          ...
          agentCapabilities: {
            ...
            // `delete` is deliberately absent: the persistence surface declares
            // no public delete, so `session/delete` is not advertised (see the
            // public-surface allow-list).
            sessionCapabilities: { list: {}, additionalDirectories: {}, close: {} },
      ...
      async newSession(params) {
        try {
          ...
          let composition
          try {
            composition = await composePreset(requestedPreset, { blank: true })
          } catch (error) {
            // composePreset has already substituted for a preset this roster no
            // longer has (a blank session has nothing to strand), so what reaches
            // here could not compose at all — the roster's own resolution failure,
            // wrapped with the roster and the fix. A live client's request keeps
            // its invalid-params shape; a deployment it named is not one.
            const detail = error instanceof Error ? error.message : String(error)
          ...
          return {
            ...
            ...permission === undefined ? {} : {
              modes: {
                currentModeId: permission.current(handle.agent.session),
                availableModes: permissionModes(permission),
              },
            },
            configOptions,
          }
      ...
      async loadSession(params) {
        ...
        if (live !== undefined) {
          ...
          return {
            ...permission === undefined ? {} : {
              modes: {
                currentModeId: permission.current(live.agent.session),
                availableModes: permissionModes(permission),
              },
            },
        ...
        let composition
        try {
          composition = await composePreset(runningPreset?.preset, { blank: runningPreset?.blank === true })
        } catch (error) {
          // A stored session keeps its preset: substituting a different
          // composition would strand the tool calls its log already carries, so
          // an unresolvable one fails here unless the log is still blank. The
          // error carries the roster and the migration (see presetFailure).
          const detail = error instanceof Error ? error.message : String(error)
          if (isPresetClientError(error)) {
            throw invalidParams(detail)
          }
      ...
      async prompt(params) {
        ...
        if (commandMatch?.[1] === 'model') {
          // Same refresh contract as /preset and registry commands: a switch
          // mutates the session selection, so the config options (model /
          // reasoning_effort currentValue) must be rebroadcast — without it
          // the editor's dropdown keeps showing the pre-switch route.
          const reply = await modelCommandText(record, commandLine.slice(commandMatch[0].length).trim())
        ...
        const stopReason = await new Promise((resolve, reject) => {
          ...
          void record.agent.whenIdle().then(() => {
            ...
            if (end === undefined) {
              ...
              // Straight codec mapping — including `max-tokens` → `max_tokens`,
              // a legal ACP stop reason the client should see (an earlier
              // rewrite to `end_turn` hid truncation and made the codec's
              // mapping a dead branch).
      ...
      async listSessions(params) {
        ...
        const persistence = ctx.get('sessionPersistence')
        const snapshots = persistence === undefined ? [] : await storedSessionSnapshots(persistence)
        const listed = []
        for (const snapshot of snapshots) {
          const header = snapshot.header
          if (cwd !== undefined && header.cwd !== undefined && header.cwd !== cwd) continue
          // Hand the reader the same recency the response reports below, so it
          // spends a short budget on the sessions a sidebar actually shows.
          snapshot.updatedAt = sessions.get(header.id)?.lastActivityAt ?? header.createdAt
          listed.push({ snapshot, header })
        }
        // One batched pass over the logs that can still yield a title, in place
        // of a `stat()` per session — see `storedTitles` for why. Only entries
        // whose title is still unknown are candidates: a live session's own
        // title is authoritative, and decoding its stored log would spend the
        // budget the bare entries need (and could report an older title over
        // the live one). Candidate positions map back to `listed` slots, which
        // is what the lookups below use.
        const candidates = []
        for (const [slot, { snapshot, header }] of listed.entries()) {
          if (sessions.get(header.id)?.title === undefined) candidates.push({ snapshot, slot })
        }
        const collected = candidates.length === 0
          ? { titles: new Map(), complete: true }
          : await storedTitles(persistence).collect(
              candidates.map(({ snapshot }) => snapshot),
              storedTitleBudgetMs)
        const storedBySlot = new Map()
        for (const [position, title] of collected.titles) {
          storedBySlot.set(candidates[position].slot, title)
        }
        // Whatever the budget could not reach keeps resolving off the request
        // path, so the client gets its list now and the remaining titles arrive
        // as `session_info_update` notifications.
        if (!collected.complete) {
          void storedTitles(persistence).resolveRemaining(candidates.map(({ snapshot }) => snapshot))
        }
        const out = new Array(listed.length)
        for (const [slot, { header }] of listed.entries()) {
          const live = sessions.get(header.id)
          const title = live?.title ?? storedBySlot.get(slot)
      ...
      // No deleteSession: the persistence surface declares no public delete, so
      // `session/delete` is neither advertised nor implemented. Persisted
      // sessions stay under $DSH_HOME/sessions until an upstream delete API
      // exists. `session/close` (above) is unaffected: it releases the live
      // handle and leaves the artifact in place.
    }
  }
