...
import { StoredTitleReader } from './stored-titles.js'
...
export function apply(ctx, config) {
   ...
   *  The deferred pass below finishes the rest off the request path. */
  const storedTitleBudgetMs = Number(process.env.DSH_ACP_LIST_BUDGET_MS ?? 2_500)
  ...
  function storedTitles(persistence) {
    if (storedTitlesReader === undefined) {
      storedTitlesReader = new StoredTitleReader({
        ...
        // client already has the list, so it renders the entry bare and fills
        // the label in when this arrives (the same update the live path uses).
    ...
    return {
      ...
      async listSessions(params) {
        ...
        for (const snapshot of snapshots) {
          ...
          // Hand the reader the same recency the response reports, so it spends
          // a short budget on the sessions a sidebar actually shows.
        ...
        // of a `stat()` per session — see `storedTitles` for why. Its positions
        // match `listed`, which is what the lookups below use.
        const listedSnapshots = listed.map(({ snapshot }) => snapshot)
        const needsStoredTitles = listed.some(({ header }) => sessions.get(header.id)?.title === undefined)
        const stored = needsStoredTitles
          ? await storedTitles(persistence).collect(listedSnapshots, storedTitleBudgetMs)
        ...
        if (!stored.complete) {
          void storedTitles(persistence).resolveRemaining(listedSnapshots)
        }
        const out = new Array(listed.length)
        for (const [slot, { header }] of listed.entries()) {
          const live = sessions.get(header.id)
          const title = live?.title ?? stored.titles.get(slot)
