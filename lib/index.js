...
export const inject = ['agents', 'llm', 'approval', 'tools', 'commands', 'skills']
...
export const Config = Schema.object({
  ...
  /** Initial agent preset every created agent is composed from, when a
 ...
 *
 * Mirrors codex-acp's command-execution presentation: shell executors
 * (`bash`, `pwsh`) and the client-terminal tool are `execute` so Zed renders
 * a terminal card. For bash/pwsh the bridge also embeds `content: [{ type:
 * 'terminal', terminalId }]` and streams the rendered output through
 * `_meta.terminal_output*` / `_meta.terminal_exit`, exactly like codex-acp,
 * so the command line and output show in the panel instead of a raw-JSON card.
 */
...
/** Whether a tool result is presented through the Zed terminal panel. */
...
/** Resolve the cwd a bash/pwsh call will run in, for the terminal_info card. */
function shellCallCwd(args, session) {
  const headerCwd = session?.header?.cwd
  if (typeof args === 'object' && args !== null && typeof args.workdir === 'string' && args.workdir.length > 0) {
    return isAbsolute(args.workdir) ? args.workdir : headerCwd === undefined ? args.workdir : resolvePath(headerCwd, args.workdir)
  }
  return headerCwd ?? process.cwd()
}

/** Build the codex-acp-style tool_call update for a bash/pwsh call. */
function shellToolCallUpdate(callId, name, rawArguments, session) {
  const args = parseToolArguments(rawArguments)
  const command = typeof args === 'object' && args !== null && typeof args.command === 'string'
    ? args.command
    : typeof rawArguments === 'string' ? rawArguments : name
  const cwd = shellCallCwd(args, session)
  return {
    sessionUpdate: 'tool_call',
    toolCallId: callId,
    title: stripShellPrefix(command) || name,
    kind: 'execute',
    status: 'in_progress',
...
/** Extract the full text of a dsh tool result (no truncation). */
 ...
 * inverse of the `[exit code: N]` / `[killed by signal: X]` markers the
 * shell tools append. Returns the marker-free body plus exit code/signal.
 */
function parseShellExitStatus(text) {
  const signal = /\n\[killed by signal: ([^\]\n]+)\]$/.exec(text)
...
/**
 * Build the codex-acp-style tool_call_update for a finished bash/pwsh call:
 * the terminal panel content is streamed via `_meta.terminal_output*` and
 * closed with `_meta.terminal_exit`, while `rawOutput` keeps the structured
 * `{ formatted_output, exit_code }` shape codex-acp emits.
 */
function shellToolCallUpdateFinished(callId, event, terminalOutputMode) {
  const text = resultText(event) ?? ''
  const { body, exitCode, signal } = parseShellExitStatus(text)
...
export function apply(ctx, config) {
   ...
   * composition failure rolls the whole creation back.
   *
   * A deployment without a roster returns an empty composition — every session
   * then shares the host composition (the pre-preset behavior).
   *
    ...
    try {
      ...
      switch (event.type) {
        ...
        case 'tool/call': {
          ...
          const shellCall = isTerminalToolName(event.data.name)
          const base = shellCall
            ? shellToolCallUpdate(event.data.callId, event.data.name, event.data.arguments, session)
            : {
              sessionUpdate: 'tool_call',
        ...
        case 'tool/result': {
          ...
          const terminal = isTerminalToolName(record.toolStats.lastName) || isTerminalToolName(event.data.name)
          const preview = resultPreview(event)
          const base = terminal
            ? shellToolCallUpdateFinished(callId, event, terminalOutputMode())
            : {
              sessionUpdate: 'tool_call_update',
              toolCallId: callId,
  ...
  async function buildConfigOptions(record) {
    ...
    if (presets !== undefined) {
      ...
      const mountable = roster.filter((preset) => preset.broken === undefined)
   ...
   * plus every user-invocable skill name so the Zed client accepts
   * `/skill-name` slashes (Zed validates against `available_commands` for ACP
   * connections — its `available_skills` only covers native agents).
   */
  async function publishCommands(record) {
    ...
    // the Zed client rejects the slash before it reaches the bridge.
    try {
  ...
  async function presetCommandText(record, query) {
    ...
    const needle = query.trim().toLowerCase()
    const matches = roster.filter((preset) => preset.id.toLowerCase().includes(needle) || preset.name?.toLowerCase().includes(needle))
  ...
  function makeRecord(handle) {
    ...
    return {
      ...
      lastUsage: undefined,
      presetId: runningPresetOf(handle.agent.session),
      selection,
    }
  }
      ...
      try {
        switch (event.type) {
          ...
          case 'tool/call': {
            ...
            const shellCall = isTerminalToolName(event.data.name)
            const base = shellCall
              ? shellToolCallUpdate(event.data.callId, event.data.name, event.data.arguments, record.agent.session)
              : {
                sessionUpdate: 'tool_call',
                toolCallId: event.data.callId,
                title: event.data.name,
            ...
            await notifyNow(record, {
              ...
              _meta: {
                ...
                step: event.data.step,
                name: event.data.name,
                argumentsPreview: String(event.data.arguments ?? '').slice(0, 200),
                ...base._meta,
              },
            })
            break
          ...
          case 'tool/result': {
            const callId = event.data.message?.content?.[0]?.toolCallId ?? event.data.callId
            const terminal = isTerminalToolName(lastToolName)
            const preview = resultPreview(event)
            const base = terminal
              ? shellToolCallUpdateFinished(callId, event, terminalOutputMode())
              : {
  ...
  const makeAgent = (connection) => {
    ...
    return {
      ...
      async newSession(params) {
        ...
        const composition = await composePreset(requestedPreset)
      ...
      async loadSession(params) {
        ...
        const composition = await composePreset(runningPreset)
      ...
      async prompt(params) {
        ...
        let text = acpPromptToText(params.prompt)
      ...
      async setSessionConfigOption(params) {
        ...
        switch (params.configId) {
          ...
          case 'agent_preset': {
            ...
            // resume re-resolves the NEW composition. Append through the session
            // object (the unbound `append` reads its private log).
            record.agent.session.append('agent-preset/selected', { agentPreset: preset.id })
            record.presetId = preset.id
            break
          }
