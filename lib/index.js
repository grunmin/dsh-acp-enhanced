...
export function apply(ctx, config) {
  ...
  ctx.on('agent/assistant-stream', ({ agent, frame }) => {
    // `start`/`end` only frame the attempt; the chunks are the stream.
    if (frame?.type !== 'chunk') return
    const record = ownedRecord(agent)
    if (record === undefined) return
    if (process.env.ACP_DEBUG) {
      process.stderr.write(`[acp-debug] agent/assistant-stream frame=chunk chunkType=${frame.chunk.type} turn=${frame.turn} step=${frame.step}\n`)
    }
    try {
      // The synthetic event carries the turn/step on `data`, which
      // `emitUsage` copies into its `_meta`.
      handleChunk(record, { data: { chunk: frame.chunk, turn: frame.turn, step: frame.step } })
    } catch (error) {
