 ...
 *   carries torn text. Two live seams feed it: the `assistant/chunk` session
 *   event on harnesses ≤ 0.1.2-rc.1, and the `agent/assistant-stream` frames
 *   event that replaced it in 0.1.3-alpha.2. A host that emits neither — or
...
export function apply(ctx, config) {
    ...
    try {
      ...
      switch (event.type) {
        case 'assistant/chunk':
          // Legacy live seam: ≤ 0.1.2-rc.1 persists every stream chunk as this
          // session event. 0.1.3-alpha.2 removed it in favour of the
          // `agent/assistant-stream` frames seam below; a host that somehow
          // carried both must not stream every block twice.
          if (record.streamSeam !== 'frames') {
            record.streamSeam = 'legacy'
  ...
  /**
   * Live assistant frames (harness ≥ 0.1.3-alpha.2).
   *
   * 0.1.3-alpha.2 removed the persisted `assistant/chunk` session event and
   * publishes the same `StreamChunk` vocabulary as a process-local,
   * agent-scoped Cordis dispatch instead: one attempt emits ordered
   * `start` / `chunk` / `end` frames, and `frame.chunk` is the very chunk the
   * old session event carried. Feeding those chunks through the same
   * `handleChunk` restores block-level streaming and `streamDeltas` on the new
   * generation; on ≤ 0.1.2-rc.1 the event never fires.
   *
  ...
  ctx.on('agent/assistant-stream', ({ agent, frame }) => {
    ...
    // Per-step latch: the first live seam to deliver owns the step, so a host
    // carrying both `assistant/chunk` and frames cannot stream it twice.
    if (record.streamSeam === 'legacy') return
    record.streamSeam = 'frames'
    if (process.env.ACP_DEBUG) {
    ...
    try {
      // The synthetic event preserves the turn/step the legacy session event
      // carried on `data`, which `emitUsage` copies into its `_meta`.
   ...
   *
   * Two live seams feed `handleChunk`: the `assistant/chunk` session event
   * (≤ 0.1.2-rc.1) and the `agent/assistant-stream` frames event
   * (≥ 0.1.3-alpha.2). This fallback is the third tier: a host that emits
   * neither — or whose frames were dropped — still delivers its reply from the
   * committed `assistant/message`, instead of settling the turn with usage
   * reported and an empty thread. It restores *delivery*, not streaming.
   *
   ...
   *  block indexes and the live-seam latch restart per step, so nothing
   *  survives a step boundary. */
  ...
  function makeRecord(handle) {
    ...
    return {
      ...
      streamedText: '',
      streamedThought: '',
      /** Which live seam owns this step: `'legacy'` (`assistant/chunk` session
       *  event, ≤ 0.1.2-rc.1), `'frames'` (`agent/assistant-stream`,
       *  ≥ 0.1.3-alpha.2), or undefined before the first chunk. Set by the
       *  first seam to deliver and cleared per step, so a host exposing both
       *  cannot stream every block twice. */
      streamSeam: undefined,
  ...
  //
  // ≤ 0.1.2-rc.1 exposed `list() → SessionHeader[]`, `load(id) → {meta, events}`
  // and (raw-artifact backends) `readRaw(id)`. 0.1.5 replaced that surface with
  // the handle API: `list() → {header, revision, sizeBytes?}[]`, `stat(id)`, and
  // `open(id, 'read') → handle` (`handle.header` + `handle.read()`), dropping
  // `load`/`readRaw`. These probes absorb both shapes per service instance —
  // never per this package's dependency range.

  /** Stored session headers, unwrapping the 0.1.5 `{header, …}` snapshot. */
  ...
  /** One stored session's header plus its full event log. */
  async function loadStoredSession(persistence, id) {
    if (typeof persistence.load === 'function') {
      const { meta, events } = await persistence.load(id)
      return { meta, events }
    }
  ...
  async function readStoredTitle(persistence, id) {
    try {
      // Raw-artifact backends (≤ 0.1.2-rc.1) answer without decoding events.
      if (persistence?.supportsRawArtifacts === true && typeof persistence.readRaw === 'function') {
        ...
        return lastTitleOf(rows)
      }
      // 0.1.5 dropped raw artifacts: read through a handle, keeping the same
      // ceiling via the stored artifact size when the backend reports it.
    ...
    return {
      ...
      async deleteSession(params) {
        ...
        if (persistence !== undefined) {
          try {
            const headers = await storedSessionHeaders(persistence)
            const header = headers.find((entry) => entry.id === sessionId)
            if (header !== undefined) {
              // `locate` is the JSONL backend's path resolver and is absent on
              // other backends; deletion is best-effort either way.
