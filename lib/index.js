...
function toolCallTitle(name, argumentsValue) {
  ...
  if (name === 'zed_terminal' || /bash|shell|exec|run_code|execute|terminal/.test(name)) {
    ...
    if (command) return clipOneLine(command, 100)
    const argv = obj.args
    if (Array.isArray(argv) && argv.length > 0) return clipOneLine(argv.map(String).join(' '), 100)
    const cwd = str('cwd')
    if (cwd) return clipOneLine(`Run in ${cwd}`, 100)
  }
